package crowplexus.cne.hscript.macros;
...
class ClassExtendMacro {
	...
	public static function init() {
		...
		for(apply in Config.ALLOWED_CUSTOM_CLASSES) {
			Compiler.addGlobalMetadata(apply, "@:build(crowplexus.cne.hscript.macros.ClassExtendMacro.build())");
		}
		#end
		#end
	}
	...
	public static function build():Array<Field> {
		...
		if (!cl.name.endsWith("_Impl_") && !cl.name.endsWith(CLASS_SUFFIX) && !cl.name.endsWith("_HSC")) {
			...
			if(fkey == "crowplexus.cne.hscript.CustomClassHandler.TemplateClass") return fields; // Error: Redefined
